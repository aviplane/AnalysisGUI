...
rawim_bg=np.load("S:\\Schleier Lab Dropbox\\Cavity Lab Data\\Cavity Lab Scripts\\cavity_analysis\\background.npy")
background_components = np.load("S:\\Schleier Lab Dropbox\\Cavity Lab Data\\Cavity Lab Scripts\\cavity_analysis\\background_components_pca.npy")
...
def get_holding_folder(apd, data_date=str(date.today())):
    fpd = "S:\\Schleier Lab Dropbox\\Cavity Lab Data\\" + \
          data_date[0:4] + "\\" + data_date[0:7] + f"\\{data_date}\\{apd}"
    return fpd

def get_date_data_path(data_date):
    s = f"S:\\Schleier Lab Dropbox\\Cavity Lab Data\\{data_date[0:4]}\\" + data_date[0:7] + f"\\{data_date}"
    return s
...
def useroi(item, xpts, ypts):
    ...
    a = np.array([item[int(ypts[0]):int(ypts[1]), int(xpts[0]):int(xpts[1])]], dtype = 'float')
...
def compute_rois_single_image(f, image):
    ...
    background_estimate = np.sum(
            background_estimate_coeffs[:, np.newaxis] * (orthogonal_masked_components), 
            axis = 0
        ).reshape(1024, 1024)
...
def get_xlabel(paths):
    ...
    xlabels = [i for i in global_df.columns if len(global_df[i].astype(str).unique()) > 1]
    return xlabels
...
#            return 'run number'
        ...
        if 'Tweezer_RamseyPhase' in labels or 'SP_RamseyPulsePhase' in labels:
            raman_ramsey = True
            a = [i for i in labels if i not in ['SP_RamseyPulsePhase', 'SP_RamseyPulseDuration']]
            if not a:
                return 'iteration'
            return a[0]
        if 'SP_A_RamseyPulsePhase' in labels:
            raman_ramsey = True
            a = [i for i in labels if i not in ['SP_A_RamseyPulsePhase', 'SP_A_RamseyPulseTime']]
            if not a:
                return 'iteration'
            return a[0]
